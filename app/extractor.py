...
_SYSTEM_PROMPT = (
    ...
    "materials/fabrics (e.g. leather, suede, denim, linen, silk, wool, corduroy), "
    "colors, patterns (e.g., stripes, plaid), and specific nicknames or styles. "
    "If the owner specifies a material or fabric, you MUST include that material in sub_category "
    "(e.g. 'leather handbag', 'linen button-down shirt') and include it in style_tags.\n"
...
def extract_garment_metadata(
    ...
    if user_caption:
        user_text = (
            ...
            "Incorporate these details (especially materials or fabrics like leather, linen, or denim, patterns, cuts, or colors) into the metadata."
        )
...
def refine_garment_metadata(
    ...
    instruction = (
        ...
        "CRITICAL: If the owner mentions a specific material, fabric, or texture (such as leather, "
        "suede, denim, linen, silk, wool, corduroy), you MUST explicitly include it in "
        "'sub_category' (e.g. 'leather handbag', 'linen shirt') and add it to 'style_tags'. "
        "If the owner notes hardware or accents (e.g. 'gold zipper'), capture the hardware color "
        "in 'accent_colors' without falsely attributing it to the primary garment color.\n\n"
        "Previous result:\n"
