...
async def fallback_command_router(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """End the profile conversation cleanly and route unexpected slash commands to their handlers."""
    context.user_data.pop(_DRAFT_KEY, None)
    message = update.message
    if message and message.text and message.text.startswith("/"):
        cmd = message.text.strip().split()[0].lower().lstrip("/")
        if cmd == "profile":
        ...
        command_map = {
            "start": start_command,
            "wardrobe": wardrobe_command,
            "style": style_command,
            "laundry": laundry_command,
            "delete": delete_command,
            "remove": delete_command,
            "help": help_command,
            "admintest": admin_test_command,
            "adminlive": admin_live_command,
        }
...
profile_conversation_handler = ConversationHandler(
    ...
    fallbacks=[
        ...
        CommandHandler("profile", profile_entry),
        MessageHandler(filters.COMMAND, fallback_command_router),
    ],
    name="profile_setup",
    persistent=False,
    per_message=False,
