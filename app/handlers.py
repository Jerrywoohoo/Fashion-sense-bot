...
def _clear_transient_chat_state(context: ContextTypes.DEFAULT_TYPE) -> None:
    """Clear temporary waiting flags (like awaiting style text or item edit) when running top-level commands."""
...
async def _send_wardrobe_ootd_view(
    ...
    # Deduplicate in view (defense-in-depth): if multiple records share identical items, display only the one with valid photo
    seen_item_sets: set[frozenset[str]] = set()
    filtered_ootds: list[dict[str, Any]] = []
    for ootd in ootds:
        items = frozenset(ootd.get("item_ids") or [])
        if items and items in seen_item_sets:
            continue
        if items:
            seen_item_sets.add(items)
        filtered_ootds.append(ootd)

    sorted_by_date = sorted(filtered_ootds, key=lambda x: (x.get("created_at") or "", x["outfit_id"]))
    display_num_by_id = {ootd["outfit_id"]: idx for idx, ootd in enumerate(sorted_by_date, 1)}

    for ootd in filtered_ootds:
        outfit_id = ootd["outfit_id"]
        display_num = display_num_by_id.get(outfit_id, outfit_id)
        ...
        caption = (
            f"📸 *OOTD #{display_num}* — _{safe_occ}_{f' ({date_str})' if date_str else ''}\n\n"
            f"{items_text}"
        )

        card_buttons = [
            [InlineKeyboardButton(f"🔗 Link Item from OOTD #{display_num}", callback_data=f"ootd_link_{outfit_id}")],
    ...
    outfit_str = "outfit" if len(filtered_ootds) == 1 else "outfits"
    await query.message.reply_text(
        f"📸 *OOTD Gallery ({len(filtered_ootds)} {outfit_str})*\nTap a button above to link duplicates or delete an outfit.",
        parse_mode=ParseMode.MARKDOWN,
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ Back to Categories", callback_data="wardrobe_menu")]]),
    )
...
def _build_wardrobe_link_source_keyboard(garments: list[dict[str, Any]], category: str) -> InlineKeyboardMarkup:
    ...
    for g in sorted_garments:
        ...
        buttons.append([InlineKeyboardButton(f"{item_id}: {desc} ({cat})", callback_data=f"wlink_src:{item_id}")])
    buttons.append([InlineKeyboardButton("⬅️ Back", callback_data=f"wardrobe_back_cat_{category}")])
    return InlineKeyboardMarkup(buttons)
...
async def delete_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if args:
        ...
        if raw_arg.startswith("ootd"):
            ootds = get_user_outfits(user_id, limit=100)
            sorted_by_date = sorted(ootds, key=lambda x: (x.get("created_at") or "", x["outfit_id"]))
            display_map = {idx: ootd for idx, ootd in enumerate(sorted_by_date, 1)}
            id_map = {ootd["outfit_id"]: ootd for ootd in ootds}

            target_ootd = None
            target_num_str = ""
            if len(args) > 1 and args[1].isdigit():
                target_num_str = args[1]
            elif "_" in raw_arg:
                target_num_str = raw_arg.split("_", 1)[1]
            elif raw_arg.removeprefix("ootd").isdigit():
                target_num_str = raw_arg.removeprefix("ootd")

            if target_num_str.isdigit():
                num = int(target_num_str)
                if num in display_map:
                    target_ootd = display_map[num]
                elif num in id_map:
                    target_ootd = id_map[num]

            if not target_ootd and len(ootds) == 1:
                target_ootd = ootds[0]

            if target_ootd:
                of_id = target_ootd["outfit_id"]
                disp_num = next((idx for idx, o in enumerate(sorted_by_date, 1) if o["outfit_id"] == of_id), of_id)
                deleted = delete_user_outfit(of_id, user_id, allowed_user_ids=allowed_user_ids)
                if deleted:
                    await message.reply_text(
                        f"🗑️ Deleted OOTD #{disp_num} and removed its unlinked items from your wardrobe.",
                        parse_mode=ParseMode.MARKDOWN,
                    )
                else:
                    await message.reply_text("⚠️ Could not delete OOTD.")
                return
            else:
                await message.reply_text("⚠️ OOTD not found. Use `/delete` to see available items.")
                return
    ...
    sorted_garments = sorted(garments, key=lambda g: (g.get("category") or "", g.get("sub_category") or ""))
    buttons: list[list[InlineKeyboardButton]] = []

    # Category header groupings
    current_cat = None
    row: list[InlineKeyboardButton] = []
    for g in sorted_garments:
        cat = (g.get("category") or "other").replace("_", " ").title()
        if cat != current_cat:
            if row:
                buttons.append(row)
                row = []
            current_cat = cat
            buttons.append([InlineKeyboardButton(f"── {cat} ──", callback_data="wardrobe_menu")])

        item_id = g["item_id"]
        subcat = (g.get("sub_category") or "item").replace("_", " ")
        color = g.get("color") or ""
        label = f"🗑️ {color} {subcat}"[:28]
        row.append(InlineKeyboardButton(label, callback_data=f"delitem_{item_id}"))
        if len(row) == 2:
            buttons.append(row)
            row = []
    if row:
    ...
    if ootds:
        ...
        display_num_by_id = {ootd["outfit_id"]: idx for idx, ootd in enumerate(sorted_by_date, 1)}
        for ootd in ootds:
            of_id = ootd["outfit_id"]
            disp_num = display_num_by_id.get(of_id, of_id)
            occ = escape_markdown(ootd.get("occasion") or "OOTD", version=1)
            buttons.append([
                InlineKeyboardButton(f"🗑️ 📸 OOTD #{disp_num} ({occ})", callback_data=f"ootd_del_{of_id}")
            ])
        sorted_ootds = sorted(ootds, key=lambda x: (x.get("created_at") or "", x["outfit_id"]))
        buttons.append([InlineKeyboardButton("── 📸 OOTD Outfits ──", callback_data="wardrobe_menu")])
        ootd_row: list[InlineKeyboardButton] = []
        for idx, o in enumerate(sorted_ootds, 1):
            occ = o.get("occasion") or "OOTD"
            label = f"🗑️ #{idx} {occ}"[:24]
            ootd_row.append(InlineKeyboardButton(label, callback_data=f"ootd_del_{o['outfit_id']}"))
            if len(ootd_row) == 2:
                buttons.append(ootd_row)
                ootd_row = []
        if ootd_row:
            buttons.append(ootd_row)
    ...
    buttons.append([InlineKeyboardButton("⚠️ Delete Entire Wardrobe", callback_data="wardrobe_clear_ask")])
    buttons.append([InlineKeyboardButton("⬅️ Back to Wardrobe", callback_data="wardrobe_menu")])
    ...
    await message.reply_text(
        "Select an item or OOTD to remove, type `/delete <item_id>`, or choose to clear all:",
        parse_mode=ParseMode.MARKDOWN,
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if pending_item_id:
        ...
        try:
            ...
            update_garment_extracted_data(pending_item_id, refined.garments[0], user_caption=feedback)
            ...
            if garment.get("is_verified") == 1:
                ...
                tags_desc = ", ".join(escape_markdown(t, version=1) for t in updated_g.style_tags) if updated_g.style_tags else "none"
            ...
            # Preserve user's existing duplicate toggle choices if already modified
            existing_active = (
                context.user_data.get(_DUPLICATE_LINKS_KEY, {}).get(capture_id)
                if (capture_id and context.user_data)
                else None
            )
            active_links: dict[str, str] = {}
            if existing_active is not None:
                stored_detected = context.user_data.get("detected_duplicates", {}).get(capture_id, {})
                for i_id, saved_id in duplicate_links.items():
                    if i_id in existing_active:
                        active_links[i_id] = existing_active[i_id]
                    elif i_id not in stored_detected:
                        # Brand new duplicate discovered during re-evaluation
                        active_links[i_id] = saved_id
            else:
                active_links = duplicate_links.copy()

            if capture_id and context.user_data is not None:
                context.user_data.setdefault(_DUPLICATE_LINKS_KEY, {})[capture_id] = active_links
                context.user_data.setdefault("detected_duplicates", {})[capture_id] = duplicate_links.copy()

            # Note: Do NOT resend duplicate comparison photos here since they were already
            # delivered when the photo was first received. Resending clutters the conversation.

            updated_g = refined.garments[0]
            updated_cat = escape_markdown(updated_g.category.replace("_", " ").title(), version=1)
            updated_raw_title = _format_item_title(
                updated_g.sub_category, updated_g.primary_color, updated_g.estimated_brand, max_words=None
            )
            updated_title = escape_markdown(updated_raw_title, version=1)
            updated_fit = escape_markdown(updated_g.silhouette_fit.replace("_", " "), version=1)
            updated_color_desc = escape_markdown(updated_g.primary_color, version=1)
            if updated_g.accent_colors:
                accents = ", ".join(escape_markdown(c, version=1) for c in updated_g.accent_colors)
                updated_color_desc += f" (accents: {accents})"
            updated_tags_desc = (
                ", ".join(escape_markdown(t, version=1) for t in updated_g.style_tags)
                if updated_g.style_tags
                else "none"
            )

            detail_card = (
                f"✅ *Updated details for `{pending_item_id}`!*\n"
                f"• *Style:* {updated_title} ({updated_cat})\n"
                f"• *Color:* {updated_color_desc}\n"
                f"• *Fit:* {updated_fit}\n"
                f"• *Tags:* {updated_tags_desc}\n\n"
            )

            await message.reply_text(
                detail_card + _format_extraction_summary(all_item_ids, all_extracted, duplicates),
                parse_mode=ParseMode.MARKDOWN,
    ...
    if capture_id:
        ...
        if any(candidates for candidates in duplicates.values()):
            await _cleanup_duplicate_comparison_photos(message.chat_id, capture_id, context)
            await _send_duplicate_comparison_photos(message, context, duplicates, capture_id=capture_id)
    ...
    has_pending_ootd = context.user_data is not None and (
        "pending_ootd_outfit_id" in context.user_data or "pending_ootd_combo" in context.user_data
    )

    fallback_outfit = None
    if not has_pending_ootd:
        recent_outfits = get_user_outfits(target_user_id)
        if recent_outfits:
            latest = recent_outfits[-1]
            if latest.get("occasion") in ("ootd", "demo ootd", ""):
                fallback_outfit = latest

    if has_pending_ootd or fallback_outfit:
        outfit_id = (
            context.user_data.pop("pending_ootd_outfit_id", None)
            if context.user_data
            else None
        )
        item_ids = (
            context.user_data.pop("pending_ootd_combo", [])
            if context.user_data
            else []
        )
        linked_item_ids = (
            context.user_data.pop("pending_ootd_linked", [])
            if context.user_data
            else []
        )
        ootd_image_path = (
            context.user_data.pop("pending_ootd_image", None)
            if context.user_data
            else None
        )
        prompt_msg_id = (
            context.user_data.pop("pending_ootd_prompt_msg_id", None)
            if context.user_data
            else None
        )

        if fallback_outfit and not outfit_id:
            outfit_id = fallback_outfit["outfit_id"]
            item_ids = fallback_outfit.get("item_ids", [])
            linked_item_ids = fallback_outfit.get("linked_item_ids", [])
            ootd_image_path = fallback_outfit.get("image_path")

        occasion_text = message.text.strip()

        if outfit_id:
            update_user_outfit_occasion(outfit_id, occasion_text, user_id=target_user_id)
        else:
        ...
        # Retrieve full outfit to know item IDs and sequential display index
        all_user_outfits = get_user_outfits(target_user_id)
        display_num = None
        for idx, o in enumerate(all_user_outfits):
            if o.get("outfit_id") == outfit_id:
                display_num = idx + 1
                if not item_ids:
                    item_ids = o.get("item_ids", [])
                if not linked_item_ids:
                    linked_item_ids = o.get("linked_item_ids", [])
                break
        if display_num is None:
            display_num = len(all_user_outfits) or 1

        safe_occ = escape_markdown(occasion_text, version=1)

        # Optionally update previous prompt message so it doesn't linger asking for occasion
        if prompt_msg_id:
            try:
                await context.bot.edit_message_text(
                    chat_id=message.chat_id,
                    message_id=prompt_msg_id,
                    text=f"✅ *OOTD #{display_num}* occasion recorded: *{safe_occ}*",
                    parse_mode=ParseMode.MARKDOWN,
                )
            except Exception:
                pass

        # Build clean item lines
        item_lines = []
        for i_id in item_ids:
            g = get_garment_by_id(i_id)
            if g:
                cat = escape_markdown(g["category"].replace("_", " ").title(), version=1)
                title = escape_markdown(
                    _format_item_title(g["sub_category"], g["color"], g.get("brand")), version=1
                )
                linked_tag = " _[Linked]_" if i_id in linked_item_ids else ""
                item_lines.append(f"• *{i_id}*: {title} ({cat}){linked_tag}")
            else:
                item_lines.append(f"• `{i_id}`")

        nav_keyboard = InlineKeyboardMarkup([
            [
                InlineKeyboardButton("📸 View in Wardrobe", callback_data="wardrobe_cat_ootd"),
                InlineKeyboardButton("👚 Wardrobe Menu", callback_data="wardrobe_menu"),
            ]
        ])

        msg_parts = [
            f"✅ *Got it!* OOTD is saved as *{safe_occ}*.\n",
            f"📸 *OOTD #{display_num}* saved to your OOTD collection:",
        ]
        if item_lines:
            msg_parts.extend(item_lines)
        msg_parts.append("\n_I'll remember this for your future outfit recommendations!_ ✨")
        ...
        try:
            ...
            plain_parts = [
                f"✅ Got it! OOTD is saved as {occasion_text}.\n",
                f"📸 OOTD #{display_num} saved to your OOTD collection:",
            ]
            for i_id in item_ids:
                g = get_garment_by_id(i_id)
                if g:
                    title = _format_item_title(g["sub_category"], g["color"], g.get("brand"))
                    cat = g["category"].replace("_", " ").title()
                    linked_tag = " [Linked]" if i_id in linked_item_ids else ""
                    plain_parts.append(f"• {i_id}: {title} ({cat}){linked_tag}")
                else:
                    plain_parts.append(f"• {i_id}")
            plain_parts.append("\nI'll remember this for your future outfit recommendations! ✨")
            await message.reply_text(
                "\n".join(plain_parts),
                reply_markup=nav_keyboard,
            )
...
_DUPLICATE_COMPARISON_MSG_IDS_KEY = "duplicate_comparison_msg_ids"
_DUPLICATE_PHOTOS_SENT_KEY = "duplicate_photos_sent"
_PENDING_CORRECTION_KEY = "pending_capture_correction"
_PENDING_ITEM_EDIT_KEY = "pending_item_correction"
...
def _format_extraction_summary(
    ...
    for item_id, garment in zip(item_ids, extraction.garments, strict=True):
        ...
        raw_title = _format_item_title(
            garment.sub_category, garment.primary_color, garment.estimated_brand, max_words=None
        )
        title = escape_markdown(raw_title, version=1)
...
async def _send_duplicate_comparison_photos(
    ...
    if capture_id and context.user_data is not None and sent_msg_ids:
        context.user_data.setdefault(_DUPLICATE_COMPARISON_MSG_IDS_KEY, {}).setdefault(
            capture_id, []
        ).extend(sent_msg_ids)
        context.user_data.setdefault(_DUPLICATE_PHOTOS_SENT_KEY, {})[capture_id] = True

    return sent_msg_ids


async def _cleanup_duplicate_comparison_photos(
    chat_id: int,
    capture_id: str,
    context: ContextTypes.DEFAULT_TYPE,
) -> bool:
    """Attempt to delete any duplicate comparison photos sent for capture_id.

    Returns True if all tracked messages were successfully deleted,
    or True if no comparison photos were ever sent for this capture.
    Returns False if comparison photos were sent but could not be deleted.
    """
    if context.user_data is None:
        return True

    store = context.user_data.get(_DUPLICATE_COMPARISON_MSG_IDS_KEY, {})
    photos_sent_store = context.user_data.get(_DUPLICATE_PHOTOS_SENT_KEY, {})
    had_photos = bool(photos_sent_store.get(capture_id)) or bool(
        isinstance(store, dict) and store.get(capture_id)
    )

    if not isinstance(store, dict):
        return not had_photos

    msg_ids = store.pop(capture_id, [])
    photos_sent_store.pop(capture_id, None)

    if not msg_ids:
        # If comparison photos were sent but we have no msg_ids to delete them, deletion wasn't possible
        return not had_photos
    ...
    for mid in msg_ids:
        try:
            ...
            logger.warning("Could not delete duplicate comparison photo %s: %s", mid, e)
            all_deleted = False
...
def _verification_keyboard(
    ...
    if det:
        for i_id, saved_id in det.items():
            if i_id in act:
                btn_text = f"🔗 [LINKED] Link {i_id} → {saved_id} (Tap to keep new)"
            else:
                btn_text = f"➕ [NEW ITEM] {i_id}: Keep as New (Tap to link)"
...
async def _delayed_process_photo_batch(
    ...
    for (img_path, caption), res in zip(photos, extracted_results):
        effective_photo_type = (
            PhotoType.OOTD
            if combined_extraction.photo_type == PhotoType.OOTD
            else res.photo_type
        )
        sub_res = GarmentExtractionResult(
            photo_type=effective_photo_type,
            overall_aesthetic=res.overall_aesthetic or combined_extraction.overall_aesthetic,
    ...
    if any(candidates for candidates in duplicates.values()):
        await _send_duplicate_comparison_photos(status_message, context, duplicates, capture_id=capture_id)

    if demo_mode:
        confirm_capture(capture_id)
        if combined_extraction.photo_type == PhotoType.OOTD or len(item_ids) > 1:
            save_user_outfit(
                user_id=effective_user_id,
                occasion="demo ootd",
                item_ids=item_ids,
                linked_item_ids=[],
                image_path=str(photos[0][0]) if photos else None,
            )
...
async def verification_callback_handler(
    ...
    if query.data.startswith("wlink_src_") or query.data.startswith("wlink_src:"):
        src_id = query.data.split(":", 1)[1] if ":" in query.data else query.data.removeprefix("wlink_src_")
        ...
        for tgt_g in candidates:
            ...
            buttons.append([InlineKeyboardButton(f"Merge into {tgt_id}: {tgt_desc} ({tgt_cat})", callback_data=f"wlink_tgt:{src_id}:{tgt_id}")])
    ...
    if query.data.startswith("wlink_tgt:") or query.data.startswith("wlink_tgt_"):
        if ":" in query.data:
            _, src_id, tgt_id = query.data.split(":", 2)
        else:
            remainder = query.data.removeprefix("wlink_tgt_")
            match = re.match(r"^(item_\d+)_(item_\d+)$", remainder)
            if match:
                src_id, tgt_id = match.group(1), match.group(2)
            else:
                parts = query.data.split("_")
                src_id = parts[2] if len(parts) > 2 else ""
                tgt_id = parts[3] if len(parts) > 3 else ""
        ...
        if ok:
            ...
            await query.message.reply_text(
                ...
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("📸 View OOTDs", callback_data="wardrobe_cat_ootd")],
                    [InlineKeyboardButton("👚 Back to Wardrobe", callback_data="wardrobe_menu")],
                ]),
            )
        else:
    ...
    if query.data.startswith("ootd_del_"):
        ...
        deleted = delete_user_outfit(outfit_id, user_id, allowed_user_ids=allowed_user_ids)
        if deleted:
            try:
                ...
                pass
            await query.message.reply_text(
                "🗑️ OOTD deleted. Newly added items from this outfit were removed from your wardrobe, while previously existing pieces were kept.",
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("📸 View OOTDs", callback_data="wardrobe_cat_ootd")],
                    [InlineKeyboardButton("👚 Back to Wardrobe", callback_data="wardrobe_menu")],
                ]),
    ...
    if query.data.startswith("ootd_link_"):
        ...
        all_ootds = get_user_outfits(user_id, limit=100)
        sorted_by_date = sorted(all_ootds, key=lambda x: (x.get("created_at") or "", x["outfit_id"]))
        display_num = next((idx for idx, o in enumerate(sorted_by_date, 1) if o["outfit_id"] == outfit_id), outfit_id)
        ...
        for i_id in item_ids:
            ...
            buttons.append([InlineKeyboardButton(f"{dup_tag}{i_id}: {desc} ({cat})", callback_data=f"wlink_src:{i_id}")])
        buttons.append([InlineKeyboardButton("⬅️ Back to OOTDs", callback_data="wardrobe_cat_ootd")])

        await query.message.reply_text(
            f"Select which item from *OOTD #{display_num}* you want to link to an existing wardrobe piece:",
            parse_mode=ParseMode.MARKDOWN,
            reply_markup=InlineKeyboardMarkup(buttons),
        )
        return
    ...
    if query.data.startswith("manlink_start_"):
        ...
        if len(garments) == 1:
            ...
            keyboard = InlineKeyboardMarkup([
                [
                    InlineKeyboardButton("👕 Tops", callback_data=f"manlink_cat:{item_id}:top"),
                    InlineKeyboardButton("👖 Bottoms", callback_data=f"manlink_cat:{item_id}:bottom"),
                ],
                [
                    InlineKeyboardButton("🧥 Outerwear", callback_data=f"manlink_cat:{item_id}:outerwear"),
                    InlineKeyboardButton("👟 Footwear", callback_data=f"manlink_cat:{item_id}:footwear"),
                ],
                [
                    InlineKeyboardButton("🎒 Accessories", callback_data=f"manlink_cat:{item_id}:accessory"),
                    InlineKeyboardButton("📸 OOTDs", callback_data=f"manlink_cat:{item_id}:ootd"),
                ],
                [
        ...
        for g in garments:
            ...
            buttons.append([InlineKeyboardButton(f"🔗 Link {i_id}: {color} {subcat}", callback_data=f"manlink_catmenu:{i_id}")])
    ...
    if query.data.startswith("manlink_catmenu:") or query.data.startswith("manlink_catmenu_"):
        item_id = query.data.split(":", 1)[1] if ":" in query.data else query.data.removeprefix("manlink_catmenu_")
        keyboard = InlineKeyboardMarkup([
            [
                InlineKeyboardButton("👕 Tops", callback_data=f"manlink_cat:{item_id}:top"),
                InlineKeyboardButton("👖 Bottoms", callback_data=f"manlink_cat:{item_id}:bottom"),
    ...
    if query.data.startswith("manlink_cat:") or query.data.startswith("manlink_cat_"):
        if ":" in query.data:
            _, item_id, cat = query.data.split(":", 2)
        else:
            remainder = query.data.removeprefix("manlink_cat_")
            if "_" in remainder:
                item_id, cat = remainder.rsplit("_", 1)
            else:
                item_id, cat = remainder, "all"
        if cat == "ootd":
            ...
            for ootd in ootds:
                oid = ootd["outfit_id"]
                disp_num = display_num_by_id.get(oid, oid)
                ...
                for g_id in ootd.get("item_ids") or []:
                    ...
                    if g:
                        ...
                        buttons.append([InlineKeyboardButton(f"OOTD #{disp_num} ({occ}) -> {g_id}: {desc} ({g_cat})", callback_data=f"manlink_do:{item_id}:{g_id}")])
            if not buttons:
        ...
        for g in garments:
            ...
            buttons.append([InlineKeyboardButton(f"🔗 Match with {g_id} ({desc})", callback_data=f"manlink_do:{item_id}:{g_id}")])
    ...
    if query.data.startswith("manlink_do:") or query.data.startswith("manlink_do_"):
        if ":" in query.data:
            _, pending_item_id, existing_id = query.data.split(":", 2)
        else:
            remainder = query.data.removeprefix("manlink_do_")
            match = re.match(r"^(item_\d+)_(item_\d+)$", remainder)
            if match:
                pending_item_id, existing_id = match.group(1), match.group(2)
            else:
                parts = query.data.split("_")
                pending_item_id = parts[2] if len(parts) > 2 else ""
                existing_id = parts[3] if len(parts) > 3 else ""
        ...
        existing_garment = get_garment_by_id(existing_id)
        existing_desc = _format_item_title(
            existing_garment.get("sub_category") or "piece",
            existing_garment.get("color") or "",
            existing_garment.get("brand"),
        ) if existing_garment else "saved item"
        safe_existing_desc = escape_markdown(existing_desc, version=1)

        target_uid = garment.get("user_id") or user_id
        cap_id = garment.get("capture_id")
        is_pending = not bool(garment.get("is_verified"))

        if is_pending and cap_id:
            # During intake review, register the duplicate link for pending capture confirmation
            if context.user_data is not None:
                context.user_data.setdefault(_DUPLICATE_LINKS_KEY, {}).setdefault(cap_id, {})[pending_item_id] = existing_id
                context.user_data.setdefault("detected_duplicates", {}).setdefault(cap_id, {})[pending_item_id] = existing_id

            await query.message.reply_text(
                f"✅ Selected: `{pending_item_id}` will be linked to `{existing_id}` ({safe_existing_desc}) upon confirmation.\n\n"
                f"Tap below to review and confirm your upload:",
                parse_mode=ParseMode.MARKDOWN,
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("✅ Review & Confirm Upload", callback_data=f"confirm_apply_{cap_id}")],
                ]),
            )
            return
        ...
        try:
            ...
            await query.message.reply_text(
                f"✅ *Successfully Linked Duplicate!*\n\n"
                f"`{pending_item_id}` has been merged into `{existing_id}` ({safe_existing_desc}).\n\n"
                f"All photo appearances, wear records, and OOTD references have been unified under `{existing_id}`.",
                parse_mode=ParseMode.MARKDOWN,
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("📸 View OOTDs", callback_data="wardrobe_cat_ootd")],
                    [InlineKeyboardButton("👚 Back to Wardrobe", callback_data="wardrobe_menu")],
    ...
    if query.data.startswith("confirm_toggle:") or query.data.startswith("confirm_toggle_"):
        if ":" in query.data:
            _, item_id, capture_id = query.data.split(":", 2)
        else:
    ...
    if query.data.startswith("confirm_apply_cap_") or query.data.startswith("confirm_link_cap_"):
        ...
        cleaned_all = await _cleanup_duplicate_comparison_photos(chat_id, capture_id, context)

        # Check if the confirmed photo was an OOTD
        is_ootd = (
            any(g.get("source_type") == "ootd" for g in garments)
            or len(garments) > 1
            or len(final_item_ids) > 1
        )
        if is_ootd:
            ootd_image = garments[0]["image_path"] if garments else None
            default_occasion = "ootd"
            for g in garments:
                if g.get("user_caption"):
                    default_occasion = g["user_caption"].strip()
                    break
            ...
            outfit_id = save_user_outfit(
                ...
                linked_item_ids=linked_ids_list,
                image_path=ootd_image,
            )
            if context.user_data is not None:
                context.user_data["pending_ootd_outfit_id"] = outfit_id
                context.user_data["pending_ootd_combo"] = final_item_ids
                context.user_data["pending_ootd_linked"] = linked_ids_list
                context.user_data["pending_ootd_image"] = ootd_image
            ...
            if cleaned_all:
                ...
                if context.user_data is not None and query.message:
                    context.user_data["pending_ootd_prompt_msg_id"] = query.message.message_id
            else:
                await query.edit_message_text(
                    msg.strip(),
                    parse_mode=ParseMode.MARKDOWN,
                ...
                if query.message:
                    ...
                    if context.user_data is not None and prompt_msg:
                        context.user_data["pending_ootd_prompt_msg_id"] = prompt_msg.message_id
            return
        ...
        if not cleaned_all and query.message:
    ...
    if query.data.startswith("confirm_cap_"):
        ...
        target_uid = garments[0].get("user_id") or (POOL_USER_ID if is_pool else user_id)
        confirm_capture(capture_id)
        _clear_duplicate_links(context, capture_id)
        ...
        # Check if the confirmed photo was an OOTD
        is_ootd = any(g.get("source_type") == "ootd" for g in garments) or len(garments) > 1
        if is_ootd:
            all_ids = [g["item_id"] for g in garments]
            ootd_image = garments[0]["image_path"] if garments else None
            default_occasion = "ootd"
            for g in garments:
                if g.get("user_caption"):
            ...
            if context.user_data is not None:
                context.user_data["pending_ootd_outfit_id"] = outfit_id
                context.user_data["pending_ootd_combo"] = all_ids
        ...
        confirm_text = f"✅ {len(garments)} item(s) confirmed and added to your wardrobe!"
        await query.edit_message_text(confirm_text)
        if not cleaned_all and query.message:
            await query.message.reply_text(confirm_text)
        return
    ...
    if query.data.startswith("delete_cap_"):
        ...
        if not cleaned_all and query.message:
            await query.message.reply_text("🗑️ Capture discarded.")
        return
...
async def cancel_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if context.user_data is not None:
        ...
        context.user_data.pop(_PENDING_ITEM_EDIT_KEY, None)
        context.user_data.pop("pending_ootd_outfit_id", None)
        context.user_data.pop("pending_ootd_combo", None)
        context.user_data.pop("pending_ootd_linked", None)
        context.user_data.pop("pending_ootd_image", None)
        context.user_data.pop("pending_ootd_prompt_msg_id", None)
