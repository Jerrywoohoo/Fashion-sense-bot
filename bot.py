...
from app.profile_flow import profile_conversation_handler, profile_entry
...
def build_application() -> Application:
    ...
    # Enable socket-level TCP keepalive to detect dropped connections proactively.
    socket_options = [(socket.SOL_SOCKET, socket.SO_KEEPALIVE, 1)]
    main_transport = httpx.AsyncHTTPTransport(retries=3, socket_options=socket_options)
    get_updates_transport = httpx.AsyncHTTPTransport(retries=3, socket_options=socket_options)

    # Main request client for sending messages, media, and handling callbacks.
    main_request = HTTPXRequest(
        ...
        pool_timeout=5.0,
        httpx_kwargs={"transport": main_transport},
    )

    # Dedicated polling client with aggressive read timeout to avoid zombie long-polling connections.
    get_updates_request = HTTPXRequest(
        ...
        write_timeout=10.0,
        pool_timeout=5.0,
        httpx_kwargs={"transport": get_updates_transport},
    )
    ...
    application.add_handler(CommandHandler("profile", profile_entry))
