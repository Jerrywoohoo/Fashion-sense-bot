...
class TestBatchAndWardrobe(unittest.TestCase):
    ...
    def test_delete_user_outfit_removes_unlinked_items_and_preserves_linked(self):
        from app.database import (
            delete_user_outfit,
            get_garment_by_id,
            get_user_outfits,
        ...
        # 1. Existing garment (e.g. from an earlier single item intake)
        g_exist = ExtractedGarment(category="top", sub_category="navy shirt", primary_color="navy")
        res_exist = GarmentExtractionResult(photo_type=PhotoType.SINGLE_ITEM, garments=[g_exist])
        exist_ids = insert_capture_garments(user_id, self.single_img, "cap_exist", res_exist)
        exist_id = exist_ids[0]
        mark_garment_verified(exist_id, True)

        # 2. Two new garments created in OOTD intake
        g_new1 = ExtractedGarment(category="bottom", sub_category="khaki pants", primary_color="khaki")
        g_new2 = ExtractedGarment(category="accessory", sub_category="black handbag", primary_color="black")
        res_ootd = GarmentExtractionResult(photo_type=PhotoType.OOTD, garments=[g_new1, g_new2])
        new_ids = insert_capture_garments(user_id, self.ootd_img, "cap_ootd", res_ootd)
        new_id1, new_id2 = new_ids[0], new_ids[1]
        mark_garment_verified(new_id1, True)
        mark_garment_verified(new_id2, True)

        # 3. Add appearance of exist_id in the OOTD image
        with _connect() as conn:
            conn.execute(
                "INSERT INTO garment_appearances (item_id, user_id, image_path, source_type) VALUES (?, ?, ?, ?)",
                (exist_id, user_id, self.ootd_img, "ootd"),
            )

        # 4. Save outfit with exist_id as linked, new_id1 & new_id2 as unlinked
        outfit_id = save_user_outfit(
            user_id=user_id,
            occasion="Business meeting",
            item_ids=[exist_id, new_id1, new_id2],
            linked_item_ids=[exist_id],
            image_path=self.ootd_img,
        )

        # Verify all 3 garments exist before deletion
        self.assertIsNotNone(get_garment_by_id(exist_id))
        self.assertIsNotNone(get_garment_by_id(new_id1))
        self.assertIsNotNone(get_garment_by_id(new_id2))

        # 5. Delete OOTD
        deleted = delete_user_outfit(outfit_id, user_id, clean_unlinked_items=True)
        self.assertTrue(deleted)

        # 6. Verify: linked garment preserved; unlinked garments deleted
        self.assertIsNotNone(get_garment_by_id(exist_id), "Linked garment MUST NOT be deleted!")
        self.assertIsNone(get_garment_by_id(new_id1), "Unlinked item 1 must be deleted!")
        self.assertIsNone(get_garment_by_id(new_id2), "Unlinked item 2 must be deleted!")

        # 7. Verify appearance of exist_id in ootd_img was removed
        with _connect() as conn:
            app_cnt = conn.execute(
                "SELECT COUNT(*) FROM garment_appearances WHERE item_id = ? AND image_path = ?",
                (exist_id, self.ootd_img),
            ).fetchone()[0]
            self.assertEqual(app_cnt, 0)

        self.assertEqual(len(get_user_outfits(user_id)), 0)

    def test_delete_user_outfit_renumbers_remaining_outfits(self):
        from app.database import delete_user_outfit, get_user_outfits, save_user_outfit, _connect

        user_id = "user_renumber_test"
        o1 = save_user_outfit(user_id, "outfit 1", ["item_1"])
        o2 = save_user_outfit(user_id, "outfit 2", ["item_2"])
        o3 = save_user_outfit(user_id, "outfit 3", ["item_3"])

        # Delete middle outfit (o2)
        deleted = delete_user_outfit(o2, user_id, clean_unlinked_items=False)
        self.assertTrue(deleted)

        outfits = get_user_outfits(user_id)
        self.assertEqual(len(outfits), 2)
        # Should be renumbered to 1 and 2
        ids = [o["outfit_id"] for o in outfits]
        self.assertIn(1, ids)
        self.assertIn(2, ids)

        # Delete remaining 2 outfits
        delete_user_outfit(1, user_id, clean_unlinked_items=False)
        delete_user_outfit(1, user_id, clean_unlinked_items=False)
        self.assertEqual(len(get_user_outfits(user_id)), 0)

        # Next inserted outfit should start back at ID 1
        o_new = save_user_outfit(user_id, "fresh outfit", ["item_new"])
        self.assertEqual(o_new, 1)

    def test_delete_garment_cleans_user_outfits_references(self):
        from app.database import (
            delete_garment,
            get_user_outfits,
            insert_capture_garments,
        ...
        user_id = "user_del_garment_test"
        g1 = ExtractedGarment(category="top", sub_category="shirt", primary_color="blue")
        g2 = ExtractedGarment(category="bottom", sub_category="pants", primary_color="black")
        res = GarmentExtractionResult(photo_type=PhotoType.SINGLE_ITEM, garments=[g1, g2])
        ids = insert_capture_garments(user_id, self.single_img, "cap_dg", res)
        id1, id2 = ids[0], ids[1]
        mark_garment_verified(id1, True)
        mark_garment_verified(id2, True)

        outfit_id = save_user_outfit(user_id, "work", [id1, id2])
        self.assertGreater(outfit_id, 0)
        ...
        outfits = get_user_outfits(user_id)
        self.assertEqual(len(outfits), 1)
        self.assertEqual(outfits[0]["item_ids"], [id1])

        # Delete id1 -> empty outfit is removed
        delete_garment(id1)
        outfits_after = get_user_outfits(user_id)
        self.assertEqual(len(outfits_after), 0)

    def test_delete_command_handles_ootd_deletion(self):
        from unittest.mock import AsyncMock, MagicMock
        from app.handlers import delete_command
        from app.database import (
            get_garment_by_id,
            get_user_outfits,
            insert_capture_garments,
        ...
        user_id = "user_cmd_del_test"
        g_unlinked = ExtractedGarment(category="bottom", sub_category="chinos", primary_color="beige")
        res = GarmentExtractionResult(photo_type=PhotoType.OOTD, garments=[g_unlinked])
        ids = insert_capture_garments(user_id, self.ootd_img, "cap_cmd", res)
        item_id = ids[0]
        mark_garment_verified(item_id, True)

        outfit_id = save_user_outfit(user_id, "casual date", [item_id], linked_item_ids=[], image_path=self.ootd_img)
        self.assertEqual(len(get_user_outfits(user_id)), 1)
        ...
        import asyncio
        asyncio.run(delete_command(update, context))

        # Outfit deleted
        self.assertEqual(len(get_user_outfits(user_id)), 0)
        # Unlinked item removed from wardrobe
        self.assertIsNone(get_garment_by_id(item_id))
        message.reply_text.assert_awaited()
        reply_text = message.reply_text.call_args[0][0]
        self.assertIn("Deleted OOTD", reply_text)
